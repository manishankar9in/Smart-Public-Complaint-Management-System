...
export function workerMatchesComplaint(worker, complaint) {
  ...
  const deptMatch = dutyMatchesCategory(worker?.department, complaint?.category);
  return deptMatch && workerLocationMatches(complaint, worker);
}
...
export function compareByPriority(a, b) {
  const weightA = PRIORITY_WEIGHTS[a.priority_level] || 0;
  const weightB = PRIORITY_WEIGHTS[b.priority_level] || 0;
