  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 md:space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <Link to={isWorker ? "/worker-dashboard" : user?.role === "admin" ? "/admin-dashboard" : "/user-dashboard"} className="btn-secondary">
          Back to Dashboard
        </Link>
      </div>
      ...
      <Card className="space-y-4">
        ...
        {complaint.address && (
          <p className="flex items-center gap-1 text-xs text-slate-500 mt-1">
            <MapPin size={13} className="text-slate-400" /> {complaint.address}
          </p>
        )}
      </Card>
      ...
      {/* Proof images — visible to all users after resolution */}
      {(complaint.worker_proof_image_url || complaint.admin_response_image_url) && (
        <Card>
          <h3 className="mb-3 font-black text-secondary">Resolution Evidence</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {complaint.worker_proof_image_url && (
              <div>
                <p className="mb-1 text-[10px] font-black uppercase tracking-widest text-slate-500">Worker Proof</p>
                <img
                  src={complaint.worker_proof_image_url}
                  alt="Worker resolution proof"
                  className="w-full rounded-2xl border border-slate-200 object-cover max-h-56"
                />
              </div>
            )}
            {complaint.admin_response_image_url && (
              <div>
                <p className="mb-1 text-[10px] font-black uppercase tracking-widest text-slate-500">Admin Verified Proof</p>
                <img
                  src={complaint.admin_response_image_url}
                  alt="Admin verified proof"
                  className="w-full rounded-2xl border border-slate-200 object-cover max-h-56"
                />
              </div>
            )}
          </div>
          {complaint.worker_note && (
            <p className="mt-3 rounded-xl border border-green-200 bg-green-50 p-3 text-xs text-green-900">
              <span className="font-bold">Worker Note:</span> {complaint.worker_note}
            </p>
          )}
        </Card>
      )}
      ...
      {/* Google Maps — Only for workers to help them navigate to the complaint location */}
      {isWorker && Number.isFinite(complaint.gps_lat) && Number.isFinite(complaint.gps_long) && (
        <Card>
          <div className="flex items-center justify-between mb-4 gap-3">
            <h3 className="font-black text-secondary">Complaint Location</h3>
            <a
              href={`https://www.google.com/maps/dir/?api=1&destination=${complaint.gps_lat},${complaint.gps_long}`}
              target="_blank"
              rel="noreferrer"
          ...
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-center">
            ...
            <p className="text-xs font-bold text-slate-700">GPS: {Number(complaint.gps_lat).toFixed(6)}, {Number(complaint.gps_long).toFixed(6)}</p>
      ...
      {isResolved && !isWorker && (
        <Card>
          <div className="flex items-center gap-3 rounded-xl border border-green-200 bg-green-50 p-4">
            ...
            <div>
              <p className="text-sm font-black text-green-900">Complaint Fully Resolved</p>
              <p className="text-xs text-green-700 mt-0.5">This complaint has been verified and closed by the administrator.</p>
            </div>
          </div>
        </Card>
      )}
