...
const WorkerDashboard = () => {
  ...
  const toggleAvailability = async () => {
    ...
    try {
      await api.put("/worker-auth/availability", { available: nextState });
      setAvailable(nextState);
      toast.success(`Status updated: ${nextState ? "Available for Missions" : "Marked Unavailable"}`);
    } catch {
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-slate-600">
                {user?.name} · <span className="font-bold text-blue-700">{user?.department || "Public Works"}</span> · {user?.city || user?.ward || "Virudhunagar"}
              </p>
              <p className="mt-0.5 text-[10px] text-slate-500">{user?.email}</p>
              {(user?.latitude || user?.longitude) && (
                <p className="mt-0.5 text-[10px] text-slate-400">
                  GPS: {user.latitude ?? "9.59"}, {user.longitude ?? "77.96"}
                </p>
              )}
            </div>
            <div className="flex items-center gap-2">
              ...
              >
                <span className={`inline-block w-2 h-2 rounded-full mr-1.5 ${available ? "bg-emerald-600 animate-pulse" : "bg-slate-400"}`} />
                {available ? "Available for Missions" : "Unavailable / Off-Duty"}
              </button>
            </div>
          </div>
