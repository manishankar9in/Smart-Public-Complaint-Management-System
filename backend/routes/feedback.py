...
async def submit_complaint_feedback(feedback: ComplaintFeedback):
    ...
    if complaint.get("status") not in ["RESOLVED", "ADMIN_VERIFIED"]:
        raise HTTPException(
            status_code=400,
            detail="Feedback can only be submitted after administrative verification of the solution."
        )
    ...
    if feedback.solved:
        await db.notifications.delete_many({"complaint_id": str(feedback.complaint_id)})
