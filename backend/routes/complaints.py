...
from services.n8n_service import trigger_n8n_complaint_event
...
async def create_complaint(complaint: ComplaintCreate, request: Request):
    try:
        ...
        # Store citizen email/name for notifications
        citizen = await db.users.find_one({"firebase_uid": complaint.firebase_uid}) or {}
        citizen_email = citizen.get("email") or getattr(complaint, "citizen_email", None)
        citizen_name = citizen.get("name") or getattr(complaint, "citizen_name", "Citizen")
        complaint_dict["citizen_email"] = citizen_email
        complaint_dict["citizen_name"] = citizen_name
        
        result = await db.complaints.insert_one(complaint_dict)
        complaint_id = str(result.inserted_id)
        complaint_dict["_id"] = complaint_id
        
        # 1. In-app notification
        try:
            await create_notification(
                user_id=complaint.firebase_uid,
                message=f"Complaint submitted for {complaint.category}. Priority: {priority['level']}. Status: Pending admin verification.",
        ...
        # 2. Brevo Email Notification (Non-blocking / fault-tolerant)
        email_sent = False
        if citizen_email:
            try:
                email_res = send_complaint_submitted_email(
                    ...
                    address=complaint_dict.get("address"),
                )
                email_sent = email_res.get("success", False)
                if not email_sent:
                    logger.warning(f"[EMAIL] Complaint submission email could not be sent: {email_res.get('error')}")
            except Exception as email_err:
                logger.error(f"[EMAIL ERROR] Unexpected error sending complaint submitted email: {email_err}")
        else:
            logger.info(f"[EMAIL] No email found for user {complaint.firebase_uid} — skipping email dispatch.")

        # 3. Optional n8n Workflow Trigger
        try:
            await trigger_n8n_complaint_event(
                complaint_data={
                    "complaint_id": complaint_id,
                    "category": complaint.category,
                    ...
                    "gps_long": complaint.gps_long,
                    "address": complaint_dict.get("address"),
                    "citizen_email": citizen_email,
                    "citizen_name": citizen_name,
                    "created_at": str(complaint_dict["created_at"]),
                },
                event="complaint_submitted",
            )
        except Exception as n8n_err:
            logger.debug(f"[n8n] Trigger skipped/failed: {n8n_err}")

        logger.info(f"Complaint created successfully: {complaint_id} (email_sent={email_sent})")
        return {
...
async def n8n_callback(payload: dict):
    """
    Webhook callback for n8n to update complaint status, priority, or worker assignment.
    """
    complaint_id = payload.get("complaint_id") or payload.get("id")
    if not complaint_id or not ObjectId.is_valid(complaint_id):
        raise HTTPException(status_code=400, detail="Valid complaint_id is required")

    db = await get_database()
    update_fields = {}

    if "priority_level" in payload:
        update_fields["priority_level"] = payload["priority_level"]
    ...
    if "workflow_status" in payload:
        update_fields["workflow_status"] = payload["workflow_status"]
    if "worker_uid" in payload:
        update_fields["worker_uid"] = payload["worker_uid"]

    if not update_fields:
        return {"status": "no_op", "message": "No valid update fields provided"}

    update_fields["updated_at"] = datetime.utcnow()
    update_fields["n8n_synced"] = True

    await db.complaints.update_one({"_id": ObjectId(complaint_id)}, {"$set": update_fields})
