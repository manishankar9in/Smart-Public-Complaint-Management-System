...
async def ensure_worker_fields_migrated():
    """Ensure all workers have department, available, latitude, and longitude fields."""
    db = db_manager.db
    try:
        from services.category_mapping import DUTY_TO_DEPARTMENT
        workers = await db["workers"].find({}).to_list(length=500)
        for w in workers:
            updates = {}
            if "available" not in w or w["available"] is None:
                updates["available"] = True
            if "department" not in w or not w["department"]:
                dept = DUTY_TO_DEPARTMENT.get(w.get("duty_position"), "Public Works")
                updates["department"] = dept
            if "latitude" not in w and w.get("city") == "Virudhunagar":
                updates["latitude"] = 9.59
            if "longitude" not in w and w.get("city") == "Virudhunagar":
                updates["longitude"] = 77.96
            if "duty_position" in w:
                await db["workers"].update_one({"_id": w["_id"]}, {"$unset": {"duty_position": ""}})
            if updates:
                updates["updated_at"] = datetime.utcnow()
                await db["workers"].update_one({"_id": w["_id"]}, {"$set": updates})
    except Exception as e:
        print(f"Worker fields migration warning: {e}")
...
async def init_collections():
    ...
    try:
        ...
        await ensure_worker_fields_migrated()
